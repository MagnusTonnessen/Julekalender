...
private fun String.isFiveOfAKind(): Boolean =
    groupBy { it }.let {
        any { c -> it[c]?.size == 5 } ||
            withJoker &&
            any { c -> c != 'J' && it[c]!!.size + (it['J']?.size ?: 0) >= 5 }
    }
...
private fun String.isFourOfAKind(): Boolean =
    groupBy { it }.let {
        any { c -> it[c]?.size == 4 } ||
            withJoker &&
            any { c -> c != 'J' && it[c]!!.size + (it['J']?.size ?: 0) >= 4 }
    }

private fun String.isFullHouse(): Boolean =
    groupBy { it }.let {
        it.size == 2 &&
            it.values.any { it.size == 3 } ||
            withJoker &&
            it.size == 3 &&
            !it['J'].isNullOrEmpty()
    }
...
private fun String.isThreeOfAKind(): Boolean =
    groupBy { it }.let {
        any { c -> it[c]?.size == 3 } ||
            withJoker &&
            any { c -> c != 'J' && it[c]!!.size + (it['J']?.size ?: 0) >= 3 }
    }
...
private fun String.isOnePair(): Boolean = groupBy { it }.size == 4 || withJoker && contains('J')
