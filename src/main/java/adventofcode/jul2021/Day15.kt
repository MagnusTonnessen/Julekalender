...
fun main() {
    val inputPartOne = File("src/Input.txt").readLines().map { it.map { c -> c.digitToInt() } }
...
fun index(
    pos: Point,
    input: List<List<Int>>,
): Int = pos.y * input[0].size + pos.x

fun isEnd(
    pos: Point,
    input: List<List<Int>>,
): Boolean = pos.y == input[0].lastIndex && pos.x == input.lastIndex

fun valid(
    pos: Point,
    input: List<List<Int>>,
): Boolean = pos.x >= 0 && pos.y >= 0 && pos.x < input[0].size && pos.y < input.size
