...
fun main() {
    val input = File("src/Input.txt").readLines().map { it.toMutableList() }
    ...
    println(
        "Part two: " +
            inputInt
                ...
                .flatMap { y -> inputInt[0].indices.map { x -> Point(x, y) } }
                .map {
                    if (input[it.y][it.x].isDigit() && input[it.y][it.x] != '9') {
                        findBasinSize(input, it.x, it.y)
                    } else {
                        0
                    }
                }.sortedDescending()
