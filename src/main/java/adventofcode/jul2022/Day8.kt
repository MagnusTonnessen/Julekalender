...
fun main() {
    val input = File("src/Input.txt").readLines().map { line -> line.toCharArray().map { it.digitToInt() } }
    (input.indices)
        .flatMap { row ->
            (input.indices).map { col -> visible(row, col, input) }
        }.count { it }
        .also { println("Part one: $it") }
    (input.indices)
        .flatMap { row -> (input.indices).map { col -> scenicScore(row, col, input) } }
        .max()
