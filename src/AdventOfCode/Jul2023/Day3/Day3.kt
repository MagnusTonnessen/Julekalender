package AdventOfCode.Jul2023.Day3

import java.io.File

fun main() {
    File("src/AdventOfCode/Jul2023/Day3/Input")
        .readLines()
        .map { it.toCharArray() }
        .run {
            var x = 0
...
fun List<CharArray>.findAdjacentNumber(y: Int, x: Int): Pair<Boolean, Int> {
...
fun List<CharArray>.isAdjacent(y: Int, x: Int): Boolean {
    for (i in -1..1) {
        for (j in -1..1) {
            if (i == 0 && j == 0) continue
