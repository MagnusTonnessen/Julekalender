package AdventOfCode.Jul2022.Day12

import java.io.File

fun main() {
    val input = File("src/AdventOfCode/Jul2022/Day12/Input").readLines().map { line -> line.toCharArray().map { it } }
...
fun findDistances(input: List<List<Char>>, start: Pair<Int, Int>): MutableMap<Pair<Int, Int>, Int> {
    ...
    while (queue.isNotEmpty()) {
        ...
        for (nPos in listOf(Pair(pos.first + 1, pos.second), Pair(pos.first - 1, pos.second), Pair(pos.first, pos.second + 1), Pair(pos.first, pos.second - 1))) {
            if (nPos.first >= 0 && nPos.first <= input.lastIndex && nPos.second >= 0 && nPos.second <= input[0].lastIndex) {
                if (distances[nPos] == null || distances[nPos]!! > distance + 1) {
                    ...
                    if (input[pos.first][pos.second] == 'S' || input[nPos.first][nPos.second].code <= input[pos.first][pos.second].code + 1) {
