package AdventOfCode.Jul2022.Day9
...
fun main() {
    val input = File("src/AdventOfCode/Jul2022/Day9/Input").readLines()
    val posKnots = mutableListOf(Pair(0, 0), Pair(0,0), Pair(0,0), Pair(0,0), Pair(0,0), Pair(0,0), Pair(0,0), Pair(0,0), Pair(0,0), Pair(0,0))
...
fun dist(head: Pair<Int, Int>, tail: Pair<Int, Int>): Int {
...
fun moveKnots(knots: MutableList<Pair<Int, Int>>) {
    ...
    knots.indices.drop(1).forEach {
        ...
        knots[it] = if (dist(currPos, prevPos) < 2) {
            currPos
        } else {
            Pair(
                when {
