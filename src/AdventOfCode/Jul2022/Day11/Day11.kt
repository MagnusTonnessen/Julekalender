package AdventOfCode.Jul2022.Day11

import java.io.File

fun main() {
    val monkeys = File("src/AdventOfCode/Jul2022/Day11/Input").readLines()
        .map { it.trim() }
        .chunked(7)
        .map { input ->
            val monkey = Monkey(
                input[1].substring(16).split(", ").map { it.toLong() }.toMutableList(),
...
class Monkey(
    ...
    private fun nextMonkey(item: Long): Int {
        return if (item % test == 0L) ifTrue else ifFalse
    }

    fun clone(): Monkey {
        return Monkey(items.toMutableList(), op, test, ifTrue, ifFalse, inspected)
    }
}