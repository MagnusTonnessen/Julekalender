package AdventOfCode.Jul2021.Day18
...
fun main() {
    val input = File("src/AdventOfCode/Jul2021/Day18/Input").readLines()
...
fun magnitude(number: String): Long {
    return if (number.first().isDigit()) {
        number.toLong()
    } else {
...
fun split(number: String, index: Int): String {
    return number.take(index) + splitNumber(number.substring(index, index + 2)) + number.drop(index + 2)
}

fun splitNumber(number: String): String {
    return "[${floor(number.toInt() / 2.0).toInt()},${ceil(number.toInt() / 2.0).toInt()}]"
}
...
fun increaseRightSide(string: String, value: Int): String {
